...
export function ServerList({
  ...
  const fetchServers = useCallback(
    async (resetSelection: boolean) => {
      platformAdapter
        .commands("list_leiting_servers")
        .then((res: any) => {
          const enabledServers = (res as IServer[]).filter(
            (server) => server.enabled !== false
          );
          //console.log("list_leiting_servers", enabledServers);
            ...
            {serverList.length > 0 ? (
              serverList.map((server) => (
                <div
                  ...
                  className={`w-full flex items-center justify-between gap-1 p-2 rounded-lg transition-colors whitespace-nowrap ${currentService?.id === server.id
                ...
                >
                  ...
                  <div className="flex flex-col items-center gap-2">
                    <span
                      className={`w-3 h-3 rounded-full ${server.health?.status
